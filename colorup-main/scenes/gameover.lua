...
function scene:create( event )
    ...
    local title = display.newText("Конец игры", _CX, 100, "assets/fonts/Galada.ttf", 40)
    title.fill = { 1, 1, 1 }
    ...
    local lblScore = display.newText("Рекорд: " .. utilities:getTmpScore(), _CX, _CY - 30, "assets/fonts/Galada.ttf", 26)
    lblScore.fill = { 1, 1, 1 }
    _grpContent:insert(lblScore)

    local lblCurrentHighscore = display.newText("Лучший рекорд: " .. utilities:getHighscore(), _CX, _CY - 5, "assets/fonts/Galada.ttf", 16)
    ...
    if isHighscore then
        local lblHighscore = display.newText("Лучший рекорд!", _CX, _CY - 80, "assets/fonts/Galada.ttf", 30)
    ...
    local lblPlay = display.newText("Заново", _CX, _CY + 84, "assets/fonts/Galada.ttf", 50)
    lblPlay.fill = { 0, 0, 0 }
    _grpContent:insert(lblPlay)

    -- Menu button
    local btnMenu = display.newText("Меню", _CX, _H - 100, "assets/fonts/Galada.ttf", 26)
    btnMenu.fill = { 1, 1, 1 }
